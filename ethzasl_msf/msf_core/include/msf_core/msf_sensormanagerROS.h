 ...
 */
   ...
   */
    ...
    void SetHLControllerStateBuffer(const sensor_fusion_comm::ExtEkf &msg)
    {
      ...
      outfile.open("/home/manh/msf.csv", std::ios_base::app); // append instead of overwrite
        ...
        const shared_ptr<EKFState_T> &state) const
    {
      ...
      if (pubCorrect_.getNumSubscribers() > 0)
      {
        ...
        else
        {
          ...
          msgCorrect_.state[0] =
              state_const.template Get<StateDefinition_T::p>()[0] - hl_state_buf_.state[0];
          msgCorrect_.state[1] =
              state_const.template Get<StateDefinition_T::p>()[1] - hl_state_buf_.state[1];
          msgCorrect_.state[2] =
              state_const.template Get<StateDefinition_T::p>()[2] - hl_state_buf_.state[2];
          msgCorrect_.state[3] =
              state_const.template Get<StateDefinition_T::v>()[0] - hl_state_buf_.state[3];
          msgCorrect_.state[4] =
              state_const.template Get<StateDefinition_T::v>()[1] - hl_state_buf_.state[4];
          msgCorrect_.state[5] =
              state_const.template Get<StateDefinition_T::v>()[2] - hl_state_buf_.state[5];

          Eigen::Quaterniond hl_q(hl_state_buf_.state[6], hl_state_buf_.state[7],
                                  hl_state_buf_.state[8], hl_state_buf_.state[9]);
          Eigen::Quaterniond qbuff_q = hl_q.inverse() * state_const.template Get<StateDefinition_T::q>();
          ...
          msgCorrect_.state[10] =
              state_const.template Get<StateDefinition_T::b_w>()[0] - hl_state_buf_.state[10];
          msgCorrect_.state[11] =
              state_const.template Get<StateDefinition_T::b_w>()[1] - hl_state_buf_.state[11];
          msgCorrect_.state[12] =
              state_const.template Get<StateDefinition_T::b_w>()[2] - hl_state_buf_.state[12];
          msgCorrect_.state[13] =
              state_const.template Get<StateDefinition_T::b_a>()[0] - hl_state_buf_.state[13];
          msgCorrect_.state[14] =
              state_const.template Get<StateDefinition_T::b_a>()[1] - hl_state_buf_.state[14];
          msgCorrect_.state[15] =
              state_const.template Get<StateDefinition_T::b_a>()[2] - hl_state_buf_.state[15];

          msgCorrect_.flag = sensor_fusion_comm::ExtEkf::state_correction;

          //outfile << msgCorrect_.state[0] << ", " << msgCorrect_.state[1] << ", " << msgCorrect_.state[2] << std::endl;
          // std::ofstream outfile;
      ...
      std::ofstream outfile;
      outfile.open("/home/manh/msf.csv", std::ios_base::app); // append instead of overwrite
      outfile << msgState.data[0] << "," << msgState.data[1] << "," << msgState.data[2] << std::endl;
      outfile.close();
